...
async function loadDB() {
    ...
    products: [
      {
        id: "prod-1",
        categoryId: "cat-1",
        name: "ยาสมุนไพรชันโรง",
        price: 50,
        description: "ภูมิปํญญาชาวบ้านเพื่อสุขภาพที่ดีของคุณ",
        imageUrl: "https://img1.pic.in.th/images/81abc5e9-70ae-414d-a079-c6b61d3942614424ae9e65e626b4.png",
        stock: [
          "สั่งซื้อสำเร็จ เช็คประวัติสั่งทั้งหมดของผู้ใช้งานได้เลย",
          "สั่งซื้อสำเร็จ เช็คประวัติสั่งทั้งหมดของผู้ใช้งานได้เลย",
          "สั่งซื้อสำเร็จ เช็คประวัติสั่งทั้งหมดของผู้ใช้งานได้เลย",
          "สั่งซื้อสำเร็จ เช็คประวัติสั่งทั้งหมดของผู้ใช้งานได้เลย",
          "สั่งซื้อสำเร็จ เช็คประวัติสั่งทั้งหมดของผู้ใช้งานได้เลย",
  ...
  {
    id: "video-demo-1",
    title: "ตัวอย่างการทำผลิตภัณฑ์ชุมชน",
    youtubeUrl: "https://www.youtube.com/watch?v=dQw4w9WgXcQ",
    description: "วิดีโอแนะนำขั้นตอนการผลิตผลิตภัณฑ์ชุมชน",
    productName: "",
    thumbnailUrl: "",
    isActive: true,
...
app.get("/api/community-videos", (req, res) => {
  const videos = Array.isArray(db.communityVideos)
    ? db.communityVideos
    : [];

  res.json(
    videos
      .filter((video: CommunityVideo) => video.isActive)
      .sort(
        (a: CommunityVideo, b: CommunityVideo) =>
...
app.get("/api/admin/community-videos", (req, res) => {
  const adminCheck = req.headers["x-user-role"];
  ...
  res.json(
    Array.isArray(db.communityVideos)
      ? db.communityVideos
      : []
  );
});
...
// Add community video
app.post("/api/admin/community-videos", (req, res) => {
  const adminCheck = req.headers["x-user-role"];
  ...
  if (!title || !youtubeUrl) {
    return res.status(400).json({
      error: "กรุณากรอกชื่อวิดีโอและลิงก์ YouTube"
    });
  }
  ...
  if (!Array.isArray(db.communityVideos)) {
    db.communityVideos = [];
  }

  db.communityVideos.push(newVideo);
...
app.put("/api/admin/community-videos/:id", (req, res) => {
  const adminCheck = req.headers["x-user-role"];
  ...
  const index = db.communityVideos.findIndex(
  ...
  const oldVideo = db.communityVideos[index];

  const youtubeUrl =
    req.body.youtubeUrl !== undefined
      ? String(req.body.youtubeUrl).trim()
      : oldVideo.youtubeUrl;
  ...
  res.json(db.communityVideos[index]);
});

// Delete community video
app.delete("/api/admin/community-videos/:id", (req, res) => {
  const adminCheck = req.headers["x-user-role"];
  ...
  const exists = db.communityVideos.some(
    (video: CommunityVideo) =>
      video.id === req.params.id
  );
  ...
  db.communityVideos = db.communityVideos.filter(
    (video: CommunityVideo) =>
      video.id !== req.params.id
  );
